...
4. **TUI 渲染（中等模块）** — 折叠视图：单行标题 + 代理状态 + 最近工具日志 + 用量行；运行中不显示文本输出，完成后在工具日志与输出摘要之间空一行。展开视图：完整任务正文 + 代理输出（Markdown 格式）。嵌套子代理内联渲染在发起它们的工具调用行下方。
...
两种视图，通过 `ctrl+o` 切换：
- **折叠运行态**：单行标题 + 最近 50 条工具调用日志 + 用量行。运行中不显示子代理文本输出；若工具调用超过 50 条，显示 `... N earlier tools` 省略行。嵌套子代理在发起它的工具调用行下方缩进显示一层进度。
- **折叠完成态**：单行标题 + 最近 50 条工具调用日志 + 空行 + 输出摘要 + 用量行。输出摘要去除代码块并保留前三个段落。
- **展开视图**：完整任务正文 + 代理输出（Markdown 格式）+ 嵌套子代理内联渲染于发起它们的工具调用行下方。

用量行采用与 `pi-powerline` 接近的顺序和缩写风格，但不显示自动压缩状态：`16.4%/272k ↑99k ↓5.1k R401k $0.850`。
